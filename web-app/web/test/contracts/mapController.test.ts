...
import { createDpirdMapController, createEcmwfMapController } from '../../src/lib/mapController'
...
function createFakeMap() {
  ...
  return {
    ...
    getLayer(id: string) { return layers.has(id) ? { id } : undefined },
    getSource(id: string) { return sources.has(id) ? { id } : undefined },
  }
}
...
describe('map controllers', () => {
  ...
  it('manages ECMWF raster layer lifecycle and update calls', () => {
    ...
    controller.add({ time: '2024-01-01T00:00:00Z', step: 'PT1H', variable: 't2m' })
    controller.update({ time: '2024-01-01T00:00:00Z', step: 'PT2H', variable: 't2m' })
    controller.remove()
