...
onlineDescribe('s3_http_range_test', () => {
  it('performs anonymous HTTPS range fetch against the real DPIRD object', async () => {
    const url = 'https://projects.pawsey.org.au/webviz/DPIRD/dpird_wa_stations.nc'
    ...
    expect(response.headers.get('accept-ranges') || response.headers.get('content-range')).toBeTruthy()
    expect(response.headers.get('content-range')).toContain(`${firstByte}-${lastByte}`)
