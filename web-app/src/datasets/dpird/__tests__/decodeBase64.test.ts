...
describe("decodeBase64FixedUTFLE", () => {
  it("decodes representative DPIRD station strings with null padding", () => {
    ...
    expect(decodeBase64FixedUTFLE(encoded, 22)).toEqual([
      "Binnu",
      "Perth Metro",
    ]);
  });
  ...
  it("rejects code point count not divisible by width", () => {
    expect(() => decodeBase64FixedUTFLE("base64:QQAAAA==", 2)).toThrow(
      /width/i,
    );
  });

  it("rejects non-positive width", () => {
    expect(() => decodeBase64FixedUTFLE("base64:QQAAAA==", 0)).toThrow(
      /width/i,
    );
  });
});
