<script lang="ts">
  ...
  let {
    sideBarState,
    onStateChange,
    ariaLabel = "Dataset main sidebar",
    style = "",
    children,
  }: Props = $props();
  ...
  function toggleCollapsed() {
    onStateChange(
      sideBarState.collapsed
        ? expandSideBar(sideBarState, window.innerWidth)
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    const nextWidthPx = clampSideBarWidth(event.clientX, window.innerWidth);
    onStateChange({
      collapsed: false,
      widthPx: nextWidthPx,
  ...
  onMount(() => {
    function clampToViewport() {
      ...
      if (nextWidthPx !== sideBarState.widthPx)
        onStateChange({
          collapsed: false,
          widthPx: nextWidthPx,
          previousWidthPx: nextWidthPx,
...
<aside
  ...
  style={`--side-bar-width: ${sideBarState.widthPx}px; ${style}`}
  aria-label={ariaLabel}
>
  <button
    class="collapse-button"
    type="button"
    aria-label={sideBarState.collapsed
      ? "Expand main sidebar"
      : "Collapse main sidebar"}
    aria-expanded={!sideBarState.collapsed}
    onclick={toggleCollapsed}>{sideBarState.collapsed ? "›" : "‹"}</button
  >
  <div class="sidebar-content" aria-hidden={sideBarState.collapsed}>
